...
const renderHeader = (
  text: RenderWelcomeHeaderProps['text'],
  size: RenderWelcomeHeaderProps['size'] = 'medium',
  fontWeight: RenderWelcomeHeaderProps['fontWeight'] = 'normal'
): React.ReactElement => {
  return (
  ...
  return (
    <>
      <View className="mt-9 self-center items-center">
        {screenType === 'welcome' &&
          renderHeader(`היי ${name},`, 'large', 'bold')}
        {screenType === 'welcome' &&
          renderHeader(`כיף שחזרת אלינו!`, 'large', 'bold')}
      </View>
      {screenType === 'otp' && (
        <>
          ...
          <View className="mt-3 self-center items-center">
            {renderHeader(`שלחנו לך קוד אימות בן 6 ספרות`, 'extraSmall')}
            {renderHeader(`למספר הטלפון שלך`, 'extraSmall')}
          </View>
        </>
      )}
    </>
